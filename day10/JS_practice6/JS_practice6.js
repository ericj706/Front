...
function findLongestWord(list){
    ...
    for(let i=1; i<=list.length-1; i++){
       if( long.length <= list[i].length){long = (list[i]);}
    }
    return long;
}
...
let total = (gainScore(), gainScore(), gainScore(), loseScore())
console.log(total)
...
function findTopStudent(){
    for(let i=0; i<=students.length-1;i++){
        if(students[i].score < students[i+1].score){best=students[i+1].score; return;}
        else{best=students[i].score}
    }
}
