...
let product = {제품: prompt("제품명 입력"), 가격 : prompt("가격 입력"), 제조사 : prompt("제조사 입력")}
console.log(product)
let product = {제품, 가격, 제조사}
...
let 비밀번호 = prompt("비번입력")
let 이름 = prompt("이름입력")
let member = {'id' : 아이디, 'password' : 비밀번호, 'name' : 이름}  // 배열내 저장된 기존 객체들과 동일한 속성으로 구성
// 만일 members(배열)내 입력받은 id가 존재하면 , 주의할점 : 배열내 특정값을 조회 -> 배열내 객체를 하나씩 조회
// 반복문 이용하여 특정 조건을 충족 했다/안했다를 확인하는 변수 = 스위치변수
let 중복체크 = false;
for( let i=0;i<=members.length-1;i++){
    let 객체 = members[i];  // memebers 배열내 i번째의 객체 꺼내기
    if(객체.id == member.id){
        중복체크 = true;    
        break;
    }
    // 주의: 반복문이 끝날때까지 모든 배열내 객체 조회 후 판단
}
if( 중복체크 == true){
    console.log('이미 사용주인 아이디입니다.')
}else{
...
for(i=0;i<=products.length-1;i++){
   if(products[i].id == 3){
    console.log(products[i]); 객체 = true;
   }
}
if(객체 == false){
...
/*문제 7: 데이터 그룹화하기
다음 team 배열을 department를 기준으로 그룹화하여, 아래 result(빈 객체)와 같은 형태로 만드시오.
최종 결과 형태 (result)
{
'개발팀': ['철수', '민수'],
'기획팀': ['영희', '지혜']
...
for(let i=0;i<=team.length-1;i++){
    if(team[i].department in result){
        ...
        // 만일 속성명이 특수문자 또는 변수명으로 구성된 경우 .대신 []사용
        // 객체의 대괄호[]는 속성명이 들어감
...
for(let i=0;i<=cart.length-1;i++){
    ...
    for(let j=0;j<=productsInfo.length-1;j++){
        let info = productsInfo[j]
        if(product.id == info.id){
            total += product.quantity * info.price;
        }
    }
}
console.log(total)
...
const votes = ['A', 'B', 'B', 'C', 'A', 'B', 'A'];  // 7번문제 참고
const result = { }  // 1. 결과 저장하는 빈 객체 생성
for(let i=0; i<=votes.length-1;i++){      // 2. 투표결과를 조회한다
    if(votes[i] in result){               // 3. i번째 값으로 result 객체내 속성명이 존재하면
        result[votes[i]] += 1             // 4. 해당 속성명에 득표수 1 증가한다
    }else{
        result[votes[i]] = 1;             // 5. 존재하지 않으면 득표수 1로 초기화
    }
}
console.log(result)
...
5. 최종적으로 완성된 HTML 테이블 문자열을 document.quertSelector().innerHTML 사용하여 화면에 출력합니다.
[ 공공데이터 open API 신청 ]
...
let html = `<table>
            <tr>
                <th>소재지</th> <th>업소명</th> <th>업태</th>
                <th>연번</th> <th>전화번호</th> <th>지정메뉴</th>
            </tr>`
