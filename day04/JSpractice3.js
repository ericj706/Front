...
if(존재여부){console.log('이미 존재하는 과일입니다.')}
else{fruitList.push(과일)
...
let 주민번호 = prompt('주민번호 입력')
console.log(주민번호)
let 성별 = 주민번호[6]
if(성별 == 1 || 성별 == 3){console.log('blue')}
else if(성별 == 2 || 성별 ==4){console.log('red')}
*/
...
let 구매금액 = Number(prompt("구매 금액 입력"))
if(구매금액>=50000) console.log('10% 할인')
else if(구매금액>=30000) console.log('5% 할인')
else if(구매금액>=10000) console.log('1% 할인')
...
// 1. max 변수 선언(가장큰수를 담는 변수 선언)
let max = 정수1
// 2. 두번째 값이 max보다 큰지 확인
if(max < 정수2){max = 정수2}
if(max < 정수3){max = 정수3}
console.log(max)
*/
...
if(연도%4 == 0 && 연도%100 != 0){console.log(`${연도}는 윤년입니다`)}
...
// 스왑(두 변수간 값을 교체) *변수의 특징 : 하나의 값만 저장*

let 정수1 = Number(prompt('정수1'))
let 정수2 = Number(prompt('정수2'))
let 정수3 = Number(prompt('정수3'))
if(정수1 > 정수2){                  // 부호를 바꾸면 내림차순으로 변경
    let temp = 정수1
    정수1 = 정수2
    정수2 = temp;
}
// 정수1 => 정수3, 비교 후 만약 정수1이 더 크다면 자리바꾸기
if(정수1>정수3){let temp = 정수1; 정수1=정수3; 정수3=temp;}
// 정수2 => 정수3, 비교 후 만약 정수2가 더 크다면 자리바꾸기
...
// 처리: 경우의 수
if (player1 == 0 || player1 == 1 || player1 == 2) {     // 1. 플레이어1 가위일때
...
let 차량번호 = prompt('차량번호 입력')

if (carArray.includes(차량번호)){console.log(locationArray[carArray.indexOf(차량번호)])}
...
*/
let courseList = ['수학', '영어', '과학', '국어']
let 제외과목 = prompt('제외하고 싶은 과목명 입력')
if(courseList.includes(제외과목)){
    courseList.splice(courseList.indexOf(제외과목),1); 
