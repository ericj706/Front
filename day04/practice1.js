...
let 약함 = 보안 < 8
let 보통 = 보안 >= 8 && 보안 < 12
if(약함){console.log('약함(8자 이상으로 설정해주세요.)')}
else if(보통){console.log('보통')}
else{console.log('강함')}
*/
...
let 예약 = Number(prompt('예약할 좌석 입력'))  // 1. 입력받기 2. 저장하기
let 결과 = seatList[예약] == 'X'        // 3. 처리 : 입력받은 번호의 인덱스 값이 'X'이면
...
if( 점수 >= 900 ){console.log('A급 경품')}
else if( 점수 < 900 && 점수 >= 700){console.log('B급 경품')}
else if( 점수 <700 && 점수 >=500){console.log('C급 경품')}
...
else if(역할 == 'editor'){console.log('콘텐츠 조회만 가능합니다.')}
...
let 주문 = Number(prompt('원하는 음료 번호 0:콜라, 1:사이다, 2:커피'))

if(주문 <0 || 주문 > 2){console.log('없는 상품입니다.')}
else{console.log( `선택하신 음료는 [${drinkNames[주문]}]입니다. 
    가격은 [${drinkPrices[주문]}원]입니다.`)}
*/
