...
각 좌석은 하나의 태그로 표시합니다.
좌석 상태에 따라 태그에 CSS 색상을 다르게 적용합니다. css예: <span style = " : ;"> 좌석 </span>
'빈좌석': color: blue;
'예약석': color: red;
...
for(i=0;i<seatStatus.length;i++){
    ...
    else{html += `<span style = "color : red;"> ${seatStatus[i]} </span>`}
...
for(i=0;i<=carNumbers.length-1;i++){
    html+= `${carNumbers[i]}: ${usageMinutes[i]}분 주차, <br />`
    let 요금 = 1000 + parseInt((usageMinutes[i]-30)/10) * 500
