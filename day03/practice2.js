...
let 밑변 = Number(prompt("밑변을 입력해주세요"))
let 높이 = Number(prompt("높이를 입력해주세요"))

let 넓이 = 높이 * 밑변 /2
console.log( `밑변 : ${밑변} , 높이 : ${높이}, 삼각형의 넓이 : ${넓이}` )

-문제 2: 섭씨를 화씨로 변환
섭씨 온도를 prompt 함수로 입력받아, 화씨 온도로 변환하여 콘솔에 출력하는 프로그램을 작성하시오.
화씨 변환 공식: (섭씨 온도 * 9 / 5) + 32

let 섭씨 = Number(prompt("섭씨온도를 입력해주세요"))
let 화씨 = (섭씨 * 9 / 5) + 32
...
let 생일 = Number(prompt("출생년도를 입력해주세요"))
let 나이 = 2026 - 생일
console.log( `현재나이 : ${나이}`)
...
let 키 = Number(prompt("키를 입력해주세요"))
let 몸무게 = Number(prompt("몸무게를 입력해주세요"))
let BMI = 몸무게 / ((키 * 키)/10000 )
console.log( `BMI지수 : ${BMI}`)

-문제 5: 관리자 확인
아이디와 이메일을 prompt 함수로 입력받습니다. 아이디가 'admin'이거나 또는 이메일이 'admin@test.com'이면 '관리자'를, 
그렇지 않으면 '일반 사용자'를 콘솔에 출력하는 프로그램을 작성하시오.

let 아이디 = prompt("아이디를 입력해주세요")
let 이메일 = prompt("이메일을 입력해주세요")
...
let 결과2 = 나이 <=29 && 나이 >=20 ? "이벤트 대상입니다" : "이벤트 대상이 아닙니다"
console.log(`결과 : ${결과2}`)
...
다음과 같이 할 일 목록이 담긴 todoList 배열이 있습니다. prompt를 사용하여 새로운 할 일을 입력받아 
배열의 마지막에 추가한 후, 변경된 배열 전체를 콘솔에 출력하는 프로그램을 작성하시오.
초기 배열: let todoList = ['장보기', '과제하기'];
힌트: 배열에 요소를 추가할 때는 .push() 메소드를 사용합니다.

let 할일 = prompt("새로운 할 일을 작성해주세요")
...
다음과 같이 대기자 명단이 담긴 waitingList 배열이 있습니다. 배열의 마지막에 있는 사람의 이름을 찾아 
"마지막 대기자는 [이름]입니다." 형식으로 콘솔에 출력하는 프로그램을 작성하시오.
초기 배열: let waitingList = ['김민준', '이서연', '박도윤', '최지우'];
힌트: 배열의 마지막 요소 인덱스는 배열.length - 1 입니다.

let waitingList = ['김민준', '이서연', '박도윤', '최지우']
let 이름 = waitingList [waitingList.length -1]
